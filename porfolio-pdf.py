...
# --- DARK THEME UI (Matching your config.toml) ---
st.markdown("""
    <style>
...
def process_upload(uploaded_file):
    ...
    # 1. Handle Visual Pages (Convert to PDF bytes)
    if ext == 'pdf':
    ...
    # 2. Handle Everything Else (Attachments: DWG, ZIP, XLSX, etc.)
    else:
...
if st.session_state.binder_items:
    ...
    if pages:
        ...
        for idx, item in enumerate(st.session_state.binder_items):
            # We iterate through all but only show UI for pages in the main grid
            if item['mode'] == 'page':
                with cols[idx % 4]:
    ...
    if attachments:
        ...
        st.info("These files will be embedded inside the PDF for the viewer to open in their native software (e.g., AutoCAD).")
    ...
    if st.button("🏗️ CONSTRUCT & DOWNLOAD PORTFOLIO"):
        with st.spinner("Building Portfolio..."):
            ...
            # 1. Merge all 'page' mode items
            for item in st.session_state.binder_items:
                if item['mode'] == 'page':
                    item_doc = fitz.open(stream=item['bytes'], filetype="pdf")
                    final_pdf.insert_pdf(item_doc)
            
            # If no pages were added, create a default cover page
            if final_pdf.page_count == 0:
                p = final_pdf.new_page()
                p.insert_text((50, 50), "Portfolio Content Summary", fontsize=20)

            # 2. Embed all 'attachment' mode items
            for item in st.session_state.binder_items:
                if item['mode'] == 'attachment':
                    final_pdf.embedded_file_add(
                        item['name'], 
                        item['bytes'],
                        filename=item['name'],
